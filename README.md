...
### 6. Блокировка автоплея хрупкая
Добавлены observer для обработки блокировки автоплея и обработка кнопки отмены на end screen. Однако не могу придумать как сделать проверку на корректность работы, поскольку и при переключении кнопки автоплея end screen отменялся.
...
В процессе исправления ошибок был найден ещё один селектор, который отвечает за отображение рекомендаций в уменьшенном формате - #related